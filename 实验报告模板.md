...
- **成员信息**：
  ...
  - 成员3：潘婉钰-2326240028-答辩PPT+仓库提交
- **完成日期**：2026-01-04
- **代码仓库地址**：https://github.com/18259668436/cloud_compute_final_exam
...
<img width="2082" height="2591" alt="exported_image (1)" src="https://github.com/user-attachments/assets/27e52c81-c904-45c8-9821-1a800bd0856f" />
...
- **问题 3**
  - 现象：测试报告里健康检查 / 主页访问显示「✗」
  - 定位过程（命令/日志/对照项）：
    <img width="554" height="257" alt="507b5e31bc61ec227e4b4d567c5f514b" src="https://github.com/user-attachments/assets/992d13af-28b5-4759-99d3-037af281c635" />
  - 根因：报告里的动态判断逻辑中，curl 访问时容器已被清理（先清理再生成报告），导致报告里的 curl 拿不到响应，误判为失败。
  - 解决方法：把「清理容器」步骤移到报告生成后
  -     vim test.sh
  - <img width="1827" height="597" alt="2a2bdac438bcda14815469f346f3ff50" src="https://github.com/user-attachments/assets/cc126afe-440d-4157-9e42-76204f4a413f" />
  - 复盘与预防：本次问题的核心是流程顺序逻辑错误：测试脚本中 “清理容器” 步骤先于 “生成测试报告” 执行，导致curl访问健康检查 / 主页时，容器已被销毁，报告误判为失败（实际核心功能是正常的）。在关键步骤（如 “清理容器”）旁添加注释，明确该步骤的执行时机要求；同时在实验文档中补充 “测试流程顺序” 的注意事项，避免后续修改脚本时再次出现顺序错误。
...
#### 12.2 一键复现
-     git clone https://github.com/18259668436/cloud_compute_final_exam
-     cd ~/cloud-final-project
-     ./start-all.sh

#### 12.3 问题解答
-  1、如何实现的镜像瘦身？
   -  **多阶段构建实现分离式瘦身（Dockerfile）**
   -  核心原理：将构建环境与运行环境完全分离，确保最终镜像仅包含运行必需的最小文件集。
   -  分离前：镜像包含gcc、g++、make等工具，单阶段构建工具+源代码+依赖+缓存=大镜像(Dockerfile.base)
   -  分离后：仅保留Python运行时库，多阶段只复制编译结果=小镜像（Dockerfile）
   -  **Alpine极简基础镜像**
   -  体积优势：Alpine Linux仅5MB，Ubuntu基础约70MB
   -  安全特性：musl libc替代glibc，默认非root用户运行
   -  包管理器：apk替代apt，工具集最小化
  
-  2、为什么选择使用镜像Debian slim？
   -  兼容性优先：确保所有 Python 包能顺利编译安装
   -  体积平衡：比完整 Ubuntu 轻 70%，比 Alpine 工具更全
   -  编译环境稳定：减少 Alpine + musl libc 可能导致的编译失败
   -  缓存优化：稳定的基础镜像提高 Docker 层缓存命中率
   -  包管理器成熟：APT 提供丰富的开发工具包（gcc, g++, make, python3-dev）
